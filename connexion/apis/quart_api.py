...
class QuartApi(AbstractAPI):
    ...
    def _set_blueprint(self):
        ...
        self.blueprint = quart.Blueprint(endpoint, __name__, url_prefix=self.base_path,
    ...
    def _add_operation_internal(self, method, path, operation):
        ...
        quart_path = flask_utils.flaskify_path(path, operation.get_path_parameter_types())
    ...
    @classmethod
    def _framework_to_connexion_response(cls, response: quart.Response, mimetype):
    ...
    def _build_response(cls, mimetype, content_type=None, headers=None, status_code=None, data=None, extra_context=None):
        if cls._is_framework_response(data):
            return quart.current_app.make_response((data, status_code, headers))
        ...
        return quart.current_app.response_class(**kwargs)  # type: quart.Response

    @classmethod
    def _serialize_data(cls, data, mimetype):
        # TODO: harmonize quart, flask and aiohttp serialization when mimetype=None or mimetype is not JSON
        ...
        if (isinstance(mimetype, str) and is_json_mimetype(mimetype)):
            ...
            warnings.warn(
                "Implicit (quart) JSON serialization will change in the next major version. "
